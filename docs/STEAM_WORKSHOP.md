...
pz-enshrouded-sleep/
├── workshop.txt
├── preview.png                 # add before first upload
├── Contents/
│   └── mods/
│       └── pz-enshrouded-sleep/
│           ├── mod.info
...
│           └── 42/
│               ├── mod.info
│               ├── poster.png  # add before first upload if used
│               └── media/
├── docs/
├── README.md
├── CHANGELOG.md
...
3. Add/verify `preview.png` at the Workshop item root using the requirements of the current Build 42 `ModTemplate` generated by the installed Project Zomboid client.
4. If using in-game mod artwork, add/verify `poster.png` (and optional `icon.png`) under the versioned `42/` directory and reference them in `42/mod.info`/root mod metadata as required by the current client.
5. Review `docs/RELEASE_CHECKLIST.md` and `COMPLIANCE.md`.
6. Recheck The Indie Stone's current Project Zomboid Modding Policy and applicable terms.
7. Make a clean copy of the repository contents into the Workshop authoring directory shown by the game client. Do **not** include `.git/` or local/private artifacts.
